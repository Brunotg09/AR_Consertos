...
import { productUrl, toSlug } from "@/lib/slugify";
...
export default function ProdutoDetalhePage() {
  ...
  useEffect(() => {
    if (product) {
      ...
      const canonicalUrl = `https://ar-consertos.vercel.app${productUrl(product.id, product.name)}`;
    ...
    try {
      ...
      if (error) {
        ...
        if (slugArray.length === 1) {
          const correctUrl = productUrl(data.id, data.name);
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            url: `https://ar-consertos.vercel.app${productUrl(product.id, product.name)}`,
            brand: {
              "@type": "Brand",
              name: "A.R Conserto",
            },
